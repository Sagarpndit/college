...
<template>
    ...
    >
        <!-- Header -->
        <label v-if="!plain" class="block mb-2 text-900 font-bold" :for="name" :class="{ 'p-error': errorMessage }">
            {{ attrs.label }}
        </label>
        ...
        <div v-if="filedType === 'profile-upload'" class="flex align-items-end py-2">
            <img :src="inputFileValue" class="surface-0 mr-2 border-circle" width="100" height="100" crossorigin="anonymous" />
            <input type="file" ref="file" accept="image/*" style="display: none" @change="handleInputChange($event)" />
            <Button v-if="!attrs.disabled" severity="contrast" size="small" @click="$refs.file.click()"> Upload Image </Button>
        </div>
